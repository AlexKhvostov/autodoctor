...
class AssistantChatService
{
    ...
    public function reply(
        ...
        $system = $prompt->body
            ...
            .$dossier
            ."\n\n## Текущий шаг диалога\n".$this->stageHint($userTurn, $vehicle !== null, (string) $session->platform);
    ...
    private function stageHint(int $userTurn, bool $hasVehicle, string $platform = 'app'): string
    {
        if ($platform === 'telegram' && ! $hasVehicle) {
            return 'Канал Telegram, карточки авто ещё нет. Собери факты: марка, модель, год, топливо, пробег, VIN по желанию. '
                .'Задай 1–3 коротких уточнения только по пустым полям. Цифры не выдумывай. '
                .'Не предлагай запись на сервис, диагностику, «что делать дальше» и не проси нажать кнопки. '
                .'Выжимку для базы отправит система отдельным сообщением — не дублируй её и не смешивай с советами.';
        }

        if ($platform === 'telegram' && $hasVehicle) {
            return 'Канал Telegram, машина уже в гараже. Если человек рассказывает о сделанной работе — собери факты: что сделали, когда, пробег, детали. '
                .'Задай 1–3 коротких уточнения только по пустым полям. Цифры и даты не выдумывай. '
                .'Если задал уточняющий вопрос — дождись ответа; не считай факт подтверждённым, пока человек не ответил или явно не сказал «не знаю» / «не помню». '
                .'Не предлагай запись на сервис, «что делать дальше» и не проси нажать кнопки. '
                .'Выжимку для журнала отправит система отдельным сообщением — не дублируй её и не смешивай с советами.';
        }
