            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (vehicle == null) ...[
                    for (final topic in [
                      context.l10n.topicSuspension,
                      context.l10n.topicPaint,
                      context.l10n.topicElectronics,
                    ])
                      PreviewListTile(
                        icon: Icons.chat_bubble_outline,
                        title: topic,
                        subtitle: context.l10n.addVehicleFirst,
                      ),
                    FilledButton.icon(
                      onPressed: () {
                    ...
                    Text(
                      context.l10n.selectedVehicleContext(
                        vehicle.make,
                        vehicle.model,
                      ),
                    ),
                    const SizedBox(height: 8),
                    AutomotivePanel(
                      child: Text(context.l10n.aiTopicsUnavailable),
                    ),
                  ],
                ],
              ),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _PreviewPage(
      ...
      action: OutlinedButton.icon(
        onPressed: () => _showTopics(context),
        icon: const Icon(Icons.forum_outlined, size: 18),
        label: Text(context.l10n.topics),
      ),
      gateMessage: context.l10n.assistantGate,
      previewChild: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          _ChatBubble(text: context.l10n.chatQuestion, outgoing: true),
          _ChatBubble(text: context.l10n.chatAnswer),
          const SizedBox(height: 12),
          AutomotivePanel(
            padding: const EdgeInsets.all(8),
            child: Semantics(
              enabled: false,
              label: context.l10n.messageFieldUnavailable,
              child: TextField(
                enabled: false,
                decoration: InputDecoration(
                  labelText: context.l10n.message,
                  hintText: context.l10n.addVehicleFirst,
      ...
      vehicleChild: vehicle == null
          ? const SizedBox.shrink()
          : Column(
              key: const Key('assistant-vehicle-content'),
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [
                Text(
                  context.l10n.selectedVehicleContext(
                ...
                AutomotivePanel(
                  emphasized: true,
                  child: Text(context.l10n.aiUnavailable),
                ),
                const SizedBox(height: 12),
                TextField(
                  key: const Key('assistant-disabled-composer'),
                  enabled: false,
                  decoration: InputDecoration(
                    labelText: context.l10n.message,
                    hintText: context.l10n.aiAvailableAfterConnection,
                    suffixIcon: const Icon(Icons.send_outlined),
                  ),
                ),
                const SizedBox(height: 6),
                Text(context.l10n.aiAvailableAfterConnection),
              ],
            ),
    );
  }
...
class _ChatBubble extends StatelessWidget {
  const _ChatBubble({required this.text, this.outgoing = false});
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.only(
            ...
            bottomLeft: Radius.circular(outgoing ? 12 : 3),
            bottomRight: Radius.circular(outgoing ? 3 : 12),
          ),
          border: Border.all(
            color: Theme.of(context).colorScheme.outlineVariant,
          ),
